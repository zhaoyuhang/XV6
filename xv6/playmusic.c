...
// 绘图函数相关操作
struct Icon contentRes[] = {{"file_icon_small.bmp",   0, 0},
...
char *calcLength(uint size) {
    ...
    int mins = size / (2 * 44100 * 2) / 60;
    int secs = size / (2 * 44100 * 2) % 60;
    int n = 0;
    do {
        result[n++] = (mins % 10) + '0';
        mins /= 10;
...
struct Icon wndRes[] = {{"close.bmp", 3, 3},
                        {"pause.bmp", WINDOW_WIDTH / 5 + 20, TOPBAR_HEIGHT + TOOLSBAR_HEIGHT - (BUTTON_HEIGHT + 3)}};
...
void drawMusicWnd(Context context) {
    ...
    fill_rect(context, 1, 1, context.width - 2, TOPBAR_HEIGHT + TOOLSBAR_HEIGHT,
              TOOLSBAR_COLOR);
    puts_str(context, "Media Player", 0, WINDOW_WIDTH / 5, 3);
...
void drawMusicContent(Context context) {
    ...
    if (style == LIST_STYLE) {
        ...
        fill_rect(context, LIST_ITEM_FILENAME, TOPBAR_HEIGHT + TOOLSBAR_HEIGHT + 1,
                  LIST_ITEM_SIZE - LIST_ITEM_FILENAME - 1, TAGBAR_HEIGHT, TAGBAR_COLOR);
        fill_rect(context, LIST_ITEM_SIZE, TOPBAR_HEIGHT + TOOLSBAR_HEIGHT + 1, LIST_ITEM_TIME - LIST_ITEM_SIZE - 1,
                  TAGBAR_HEIGHT, TAGBAR_COLOR);
        fill_rect(context, LIST_ITEM_TIME, TOPBAR_HEIGHT + TOOLSBAR_HEIGHT + 1, context.width - LIST_ITEM_TIME - 1,
                  TAGBAR_HEIGHT, TAGBAR_COLOR);
        ...
        puts_str(context, "Time", 0, LIST_ITEM_TIME + 3, TOPBAR_HEIGHT + TOOLSBAR_HEIGHT + TAGBAR_HEIGHT - 18);
    }
}
...
Rect getPos(Context context, int n) {
    if (style == ICON_STYLE) {
        ...
        return initRect(0, TOPBAR_HEIGHT + TOOLSBAR_HEIGHT + TAGBAR_HEIGHT + n * (LIST_ITEM_HEIGHT + LIST_ITEM_GAP),
                        context.width,
                        LIST_ITEM_HEIGHT);
    }
}
